...
def draw_one_biased(state: dict, profile: PlayerProfile) -> dict:
    """Draw a single card with light, user-configurable bias.

    Bias rules (requested):
      - Redundancy increases the chance of drawing RED cards
      - Bluerocity increases the chance of drawing BLUE cards
      - Greenality increases the chance of drawing GREEN cards
      - Yellowtude increases the chance of drawing YELLOW cards
      - Chaos increases the chance of drawing WILD / WILD+4 cards

    This does not change the *deck composition*—it only influences which card we try to pick
    from the existing deck on a given draw, falling back to a normal draw if no match exists.
    """
    reshuffle_if_needed(state)

    weights = profile.weights or {}
    chaos = max(0, int(weights.get("Chaos", 0)))

    # How often we attempt a biased pull instead of a plain top-of-deck draw.
    # Stronger "single-color" weight => more bias.
    color_items = [
        max(0, int(weights.get("Redundancy", 0))),
        max(0, int(weights.get("Bluerocity", 0))),
        max(0, int(weights.get("Greenality", 0))),
        max(0, int(weights.get("Yellowtude", 0))),
    ]
    max_color = max(color_items) if color_items else 0
    bias_prob = min(0.85, 0.15 + 0.70 * (max_color / 100.0))  # ~0.15..0.85

    # Chaos: chance that the bias (if triggered) targets wilds.
    wild_bias = min(0.75, 0.75 * (chaos / 100.0))  # 0..0.75

    if random.random() > bias_prob:
        return state["deck"].pop()

    # 1) Chaos bias: prefer wild4, then wild.
    if random.random() < wild_bias:
        # Prefer WILD+4 more as Chaos increases
        prefer_wild4 = random.random() < min(0.85, chaos / 100.0)
        targets = ("wild4", "wild") if prefer_wild4 else ("wild", "wild4")
    ...
    # 2) Color bias: pick desired color according to the 4 weight keys.
    desired_color = weighted_color_choice(weights)
    for i in range(len(state["deck"]) - 1, -1, -1):
        c = state["deck"][i]
...
# =========================
# REDDIT CHARACTER INSIGHT (separate mode)
# =========================
def _basic_text_signals(texts: List[str]) -> Dict[str, float]:
    """Lightweight heuristics (no external NLP libs) for UI hints."""
    ...
    q = sum(t.count("?") for t in texts)
    e = sum(t.count("!") for t in texts)
    caps = sum(1 for ch in joined if ch.isalpha() and ch.isupper())
    letters = sum(1 for ch in joined if ch.isalpha())
    return {
        "n_comments": int(len([t for t in texts if t])),
        "chars": int(len(joined)),
        "avg_len": float(sum(lens) / max(1, len(lens))),
        "question_ratio": float(q / max(1, len(joined))),
        "exclaim_ratio": float(e / max(1, len(joined))),
        "caps_ratio": float(caps / max(1, letters)),
    }

def _call_character_insight_gemini(
    user_texts: List[str],
    cluster_id: int,
    class_name: str,
    ...
    """Ask Gemini for *speculative* character insights (non-diagnostic, non-identity)."""
    response_schema = {
        "type": "object",
        "properties": {
            "one_liner": {"type": "string"},
            "communication_style": {"type": "array", "items": {"type": "string"}},
            "likely_motivations": {"type": "array", "items": {"type": "string"}},
            "strengths": {"type": "array", "items": {"type": "string"}},
            "blind_spots": {"type": "array", "items": {"type": "string"}},
            "how_to_engage": {"type": "array", "items": {"type": "string"}},
    ...
    prompt_obj = {
        "task": "reddit_commenter_character_insight",
        "constraints": [
            "Return STRICT JSON only matching the schema.",
            "Be clear that this is probabilistic and based ONLY on the provided text.",
            "Do NOT claim to know the person's real identity, age, gender, race, sexuality, nationality, religion, or any protected trait.",
            "Do NOT diagnose mental/medical conditions, addiction, or neurodivergence.",
            "Avoid defamatory claims. Use cautious language like 'may', 'often', 'suggests'.",
            "Keep it useful: focus on communication style, likely intent, and how to respond productively.",
            f"Tone: {tone}.",
        ],
        "observations": {
        ...
        "user_inputs": user_texts[:10],
    }
    return call_hackathon_gemini(prompt_obj, response_schema)
...
if st.session_state.stage == "profiling":
    ...
    # Store per-human profiling outputs so we can show results (incl. character insight) BEFORE moving on.
    if "profiling_results" not in st.session_state:
        st.session_state.profiling_results = {}

    res = st.session_state.profiling_results.get(idx)

    if res:
        st.success(f"{name} is **{res['class_name']}**")
        if res.get("conf") is not None:
            st.caption(f"Confidence: {res['conf']:.2f}")

        st.info(f"🖐️ Starting hand size (Privilege): **{res['privilege']}**")
        st.json(res["weights"])

        # Show WHY / archetype explanation (from the main game Gemini call)
        why = res.get("why", [])
        if why:
        ...
        # Show roasts (still used by the game)
        roasts = res.get("roasts", [])
        if roasts:
            st.markdown("### One roast")
            st.success(random.choice(roasts))

        # Character insight (the thing you asked for) — directly after the player provides their text
        insight = res.get("character_insight")
        if insight:
            with st.expander("🧠 Character insight (speculative) — how this commenter comes across", expanded=True):
                st.caption(
                    "Based only on the pasted text. This can be wrong. It does not identify the person, and avoids protected-trait guesses and diagnoses."
                )
                st.markdown("#### One-liner")
                st.info(str(insight.get("one_liner", "")))

                c1, c2 = st.columns(2)
                with c1:
                    st.markdown("#### Communication style")
                    for b in insight.get("communication_style", [])[:12]:
                        st.write("• " + str(b))

                    st.markdown("#### Likely motivations")
                    for b in insight.get("likely_motivations", [])[:12]:
                        st.write("• " + str(b))

                with c2:
                    st.markdown("#### Strengths")
                    for b in insight.get("strengths", [])[:12]:
                        st.write("• " + str(b))

                    st.markdown("#### Blind spots")
                    for b in insight.get("blind_spots", [])[:12]:
                        st.write("• " + str(b))

                st.markdown("#### How to engage / respond")
                for b in insight.get("how_to_engage", [])[:12]:
                    st.success(str(b))

                with st.expander("Uncertainty notes"):
                    for b in insight.get("uncertainty_notes", [])[:12]:
                        st.write("• " + str(b))

        col1, col2 = st.columns([1, 1])
        if col1.button("Continue to next player", type="primary", use_container_width=True):
            st.session_state.profiles.append(
                PlayerProfile(
                    ...
                    class_id=int(res["cluster_id"]),
                    class_name=res["class_name"],
                    roasts=res.get("roasts", []),
                )
            )
            # Clear cached result + textareas for this player
            st.session_state.profiling_results.pop(idx, None)
            for i in range(1, 6):
                st.session_state.pop(f"profile_{idx}_{i}", None)
            st.session_state.profiling_idx += 1
            st.rerun()

        if col2.button("Redo (paste different text)", use_container_width=True):
            st.session_state.profiling_results.pop(idx, None)
            st.rerun()
        ...
        colA, colB = st.columns([1, 1])
        if colA.button("Generate my class + abilities + insight", type="primary", use_container_width=True):
            ...
            with st.spinner("Calling Gemini for archetype + passives + roast..."):
            ...
            # Character insight (speculative) — uses the same inputs, after the player provides their text
            with st.spinner("Generating character insight..."):
                character_insight = _call_character_insight_gemini(
                    ...
                    privilege=int(privilege),
                    conf=conf,
                    tone="Neutral",  # keep consistent for game flow
                )

            st.session_state.profiling_results[idx] = {
                "cluster_id": int(cluster_id),
                "conf": conf,
                "class_name": class_name,
                "weights": weights,
