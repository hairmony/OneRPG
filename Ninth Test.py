...
def do_clickbait(state: dict, actor: str, chosen_color: str):
    ...
    if pile_color != chosen_color:
        state["log"].append(f"{actor} tried Clickbait with {chosen_color} but pile is {pile_color}. No effect.")
        return
...
# STREAMLIT APP
# =========================
st.set_page_config(page_title="Reddit UNO Archetype Game", layout="centered")
st.title("Reddit UNO Archetype Game")
...
if st.session_state.stage == "setup_review":
    ...
    for i in range(bots_needed):
        ...
        themed = [
            f"You’re doing {class_name} cosplay and forgot the script.",
            f"Classic {class_name} move: confident, incorrect, and loud about it.",
            f"{class_name} energy: ‘I read one thread and became a philosopher.’",
            f"{class_name} mode activated: unnecessary certainty detected.",
            f"You play like a {class_name} who just discovered the caps lock key.",
            f"{class_name} behavior: turning UNO into a debate club.",
            "You’d fact-check UNO mid-game.",
            "You are the reason ‘mute chat’ exists (in spirit).",
        ]
    ...
    if turn_profile.class_id in ULT_INFO and not state["ultimate_used"][turn_name]:
        ...
        with st.expander(f"🔥 Ultimate (once per game): {ult_name}"):
            ...
            if turn_profile.class_id == 3:
                st.write(f"Pile color is: **{pile_color}**")
                color = st.selectbox("Choose color to clickbait (must match pile color)", COLORS, key="ult_cb_color")
